...
### 1、按照home-robot readme配置环境，注意不是实机部署的步骤
...
### 2、按照home-robot sim readme配置环境

    step3：python -m pip install "git+https://github.com/facebookresearch/pytorch3d.git"可能会报错，可以查看pip list，只要有pytorch3d即可



### 3、推荐先配置objectnav感受一下

    在装好habitat lab后会将numpy版本进行一个升级导致后面bug，推荐再次打一遍pip install -e src/home_robot_sim进行降级
    其他依据readme
