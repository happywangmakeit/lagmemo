...
from home_robot.mapping.semantic.categorical_2d_semantic_map_module_lagmemo import (
...
class LagMemoAgentModule(nn.Module):
    ...
    def forward(
        ...
        if matches is not None or confidence is not None:
            ...
            if len(all_matches) > 0:
                import pdb
                pdb.set_trace()
                print("20250227 永远不可能进这个分支，因为matches有值，all_matches此时一定空")
