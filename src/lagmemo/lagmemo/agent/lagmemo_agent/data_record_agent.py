...
class DataRecordAgent(Agent):
    ...
    def act(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        ...
        output_record_path = "/home/zht/github_play/mycode/data_for_gs/"
        output_vis_path = "/home/zht/github_play/mycode/home_robot_obs/"
        from PIL import Image
        import os
        def keep_img(img, path, type = ''):
            # if type == 'depth':
        ...
        current_position = obs.globalpose[4:7]
        current_rotation = obs.globalpose[0:4]
        concat_pos = obs.globalpose
        concat_pos = [str(x) for x in concat_pos]
        current_rgb = obs.rgb
        current_depth = obs.depth

        concat_pos = f"{int(self.total_timesteps[0]+1)} "+' '.join(concat_pos)
        rgb_path = output_record_path + f"{self.current_task_idx}/rgb/"
        dep_path = output_record_path + f"{self.current_task_idx}/depth/"
        pos_path = output_record_path + f"{self.current_task_idx}/"
        ...
        keep_img(current_rgb,rgb_path + f"img{str(self.total_timesteps[0]+1).zfill(4)}.png")
        np.save(dep_path + f"img{str(self.total_timesteps[0]+1).zfill(4)}.npy", np.array(current_depth))
        ...
        plt.imsave(output_vis_path + "rgb_" + str(self.total_timesteps[0]) + ".png", image_rgb)
        max_depth = 5
        min_depth = 0.5
        # to vis, normalize
        image_depth[image_depth>max_depth] = max_depth
        image_depth[image_depth<min_depth] = min_depth
        normalized_depth = (image_depth-min_depth)/(max_depth-min_depth)
        plt.imshow(normalized_depth, cmap='gray')
        plt.savefig(output_vis_path + "depth_" + str(self.total_timesteps[0]) + ".png")
