...
from home_robot.agent.goat_agent.lagmemo_agent import LagMemoAgent
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--habitat_config_path",
        type=str,
        default="projects/habitat_goat/configs/habitat/habitat_config_lagmemo.yaml",
        # 原本路径goat/modular_goat_hm3d.yaml 在habitat-lab库中
        help="Path to config yaml",
    )
    parser.add_argument(
        "--baseline_config_path",
        type=str,
        default="projects/habitat_goat/configs/agent/hm3d_lagmemo_eval.yaml",
        help="Path to config yaml",
    )
    parser.add_argument(
        "opts",
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        help="Choose which scene episode or use all"
    )
    # print("Arguments:")
    ...
    config.habitat.environment.iterator_options.scene_name = args.scene_name
    agent = LagMemoAgent(config=config)
    ...
    metrics = {}
    # success_data = {}
    # 修改env.habitat_env.episodes
    # import pdb
    # pdb.set_trace()
    print("想看metrics的success判定")
    for i in range(len(env.habitat_env.episode_iterator.episodes)):
        metrics = {}
        env.reset()
        agent.reset()
        ...
        t = 0
        # 记录目前的场景有多少个episodes
        episodes_num = np.sum([episode.scene_id == env.habitat_env.current_episode.scene_id for episode in env.habitat_env.episodes])
        scene_id = env.habitat_env.current_episode.scene_id.split("/")[-1].split(".")[0]
        print("当前的scene是：", scene_id, "当前scene有", episodes_num, "个episodes")
        ...
        print(f"目前是episode_id是：{episode_id}")
        ...
        # subtask_num可以通过agent.current_task_idx得到
        subtask_num = 0
        while not env.episode_over:
            ...
            # TODO 5 确认： 这两行得放在前面，否则最后一步时，agent.current_task_idx已经更新
            pbar.set_description(
                f"Action: {str(action).split('.')[-1]}"
            )
            pbar.update(1)

            if action == DiscreteNavigationAction.STOP:
                print(f"这里scene {scene_id} episode{episode_id} subtask{subtask_num}结束了")
                subtask_num += 1
        ...
        # TODO需要在这里进行显示的memory释放，完成一个episode的了
